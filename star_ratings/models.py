...
class AbstractBaseRating(models.Model):
    ...
    """
    count = models.PositiveIntegerField(default=0,verbose_name='تعداد نظر')
    total = models.PositiveIntegerField(default=0,verbose_name='جمع ستاره ها')
    average = models.DecimalField(max_digits=6, decimal_places=3, default=Decimal(0.0),verbose_name='میانگین')

    content_type = models.ForeignKey(ContentType, null=True, blank=True, on_delete=models.CASCADE,related_name='stars')
    ...
    class Meta:
        ...
        abstract = True
        verbose_name = 'رتبه بندی محصول'
        verbose_name_plural = 'رتبه بندی محصول ها'
...
class UserRating(TimeStampedModel):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, blank=True, null=True, on_delete=models.CASCADE,verbose_name='کاربر')
    ip = models.GenericIPAddressField(blank=True, null=True,verbose_name='ip ادرس')
    score = models.PositiveSmallIntegerField(verbose_name='امتیاز')
    rating = models.ForeignKey(get_star_ratings_rating_model_name(), related_name='user_ratings', on_delete=models.CASCADE,verbose_name='محصول')

    objects = UserRatingManager()

    class Meta:
        unique_together = ['user', 'rating']
        verbose_name = "رتبه بندی کاربر"
        verbose_name_plural = 'رتبه بندی کاربران'

    def __str__(self):
        if not app_settings.STAR_RATINGS_ANONYMOUS:
            return '[ {} ] - {} ستاره برای ({})'.format(self.user, self.score, self.rating.content_object)
        return '{} نظردهی {} برای {}'.format(self.ip, self.score, self.rating.content_object)
